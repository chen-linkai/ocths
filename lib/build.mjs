...
/* ---------- 网站根路径（用于 404 等无法用相对路径定位的页面） ----------
 * cfg.root 形如 "/ocths/" 或 "/"。为空表示不做绝对定位（<base> 不注入）。 */
function normRoot(r) {
...
/* ---------- 顶层页采用“目录 / index.html”形式，URL 为 /about/、/contact/、/terms/、/search/ ----------
 * news/ 下的栏目/详情页保持原文件名，不做改写。 */
function topDir(key) {
...
/* 由当前位置前缀 pre 生成一组链接：
 *   home  回到站点根（干净 URL）
 *   page  顶层页（about/contact/terms/search）
 *   cat   news/ 下的栏目页 */
...
function shellHtml(cfg, o) {
  ...
  const brand =
    (existsSync(logoFile)
      ? '<img class="brand-logo" src="' + pre + 'img/logo.png" alt="' + esc(cfg.siteShort) + '">'
  ...
  // 404 会被服务器用于任意路径，注入 <base> 固定站点根（config.root），否则 CSS/链接会错乱
  ...
  return fill(tpl, {
    TITLE: o.title,
    META_DESC: o.desc,
    FAVICON: pre + "favicon.ico",
    CSS: pre + "css/style.css",
    PAGE: o.page,
    BASE: baseTag,
    BRAND: brand,
    HOME: refs.home,
...
/* 顶层页输出在 about/、contact/、terms/ 等目录（URL 为 /about/ 等），pre 为 "../" */
...
/* 搜索结果页：位于 search/index.html。文章在站点根 news/…，注入的数据 URL 加 "../" 前缀。 */
...
function main() {
  ...
  // 顶层页输出为 about/index.html、contact/index.html、terms/index.html …
