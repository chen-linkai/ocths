...
function navHtml(cfg, pages, refs, active) {
  ...
  html += li(refs.news, "新闻资讯", active === "news", false) + "</li>";
...
function dotList(posts, pre, shortDate) {
  return '<ul class="dot-list">' + posts.map(p =>
    '<li><a href="' + pre + p.url + '">' +
    '<span class="txt">' + esc(p.title) + "</span>" +
    '<span class="date">' + esc(shortDate ? p.dateDisplay.slice(5) : p.dateDisplay) + "</span></a></li>"
  ).join("") + "</ul>";
}
...
function newsPage(cfg, posts, pages) {
  const pre = "../";
  const content =
    pageBanner("新闻资讯", "共 " + posts.length + " 篇") +
    '<section class="section"><div class="container">' +
    (posts.length ? '<div class="dot-panel">' + dotList(posts, pre, false) + "</div>" : '<div class="empty-state">暂无内容。</div>') +
    "</div></section>";
  return shellHtml(cfg, {
    page: "list", pre, pages, title: "新闻资讯 - " + cfg.siteFull,
    desc: cfg.siteFull + " 新闻资讯", content,
  });
}
...
function articlePage(cfg, post, pages) {
  ...
  const content =
    ...
    '<div class="article-actions"><a class="btn btn-ghost" href="' + makeRefs(pre).news + '">← 返回新闻资讯</a>' +
...
function searchPage(cfg, posts, pages) {
  ...
  const script = [
    ...
    "    res.innerHTML = '<div class=\"dot-panel\"><ul class=\"dot-list\">' + list.map(function(p){",
    "      return '<li><a href=\"'+p.url+'\">'+",
    "        '<span class=\"txt\">'+strip(p.title)+'</span>'+",
    "        '<span class=\"date\">'+strip(p.date)+'</span></a></li>';",
    "    }).join('') + '</ul></div>';",
    "  }",
...
function main() {
  ...
  write(path.join(OUT_DIR, "news", "index.html"), newsPage(cfg, posts, pages));
