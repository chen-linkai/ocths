...
function loadPosts(cfg) {
  ...
  for (const f of readdirSync(POSTS_DIR).sort()) {
    ...
    if (!date) { console.warn("  [警告] 无法解析日期，跳过：" + f); continue; }
    ...
    const contentHtml = md(body || "*（正文待补充）*");
    ...
    posts.push({
      ...
      title, author, image, summary, contentHtml, dateDisplay,
...
function navHtml(cfg, pages, pre, active) {
  const li = (url, label, isActive, hasSub) =>
    '<li class="' + (hasSub ? "has-sub " : "") + (isActive ? "active" : "") + '"><a ' +
    (hasSub ? 'class="top" ' : "") + 'href="' + pre + url + '">' + esc(label) + "</a>";
  const sub = (url, label) => "<li><a href=\"" + pre + url + '">' + esc(label) + "</a></li>";

  let html = '<ul class="navlist">';

  html += li("index.html", "首页", active === "home", false) + "</li>";

  const aboutPages = (cfg.pages || []).filter(p => p.key !== "contact");
  if (aboutPages.length > 1) {
    html += li(aboutPages[0].key + ".html", "走进侨高", active === "about", true) +
      '<ul class="subnav">' + aboutPages.map(p => sub(p.key + ".html", p.label)).join("") + "</ul></li>";
  } else if (aboutPages.length === 1) {
    const p = aboutPages[0];
    html += li(p.key + ".html", p.label, active === "about", false) + "</li>";
  }

  const first = (cfg.categories || [])[0] || { key: "news" };
  html += li("news/cat-" + first.key + ".html", "新闻资讯", active === "news", true) +
    '<ul class="subnav">' + (cfg.categories || []).map(c => sub("news/cat-" + c.key + ".html", c.label)).join("") + "</ul></li>";

  const contact = (cfg.pages || []).find(p => p.key === "contact");
  if (contact) html += li(contact.key + ".html", contact.label, active === "contact", false) + "</li>";
...
function shellHtml(cfg, o) {
  ...
  const logoFile = path.join(ROOT, "img", "logo.png");
  const brand =
    (existsSync(logoFile)
      ? '<img class="brand-logo" src="' + o.pre + 'img/logo.png" alt="' + esc(cfg.siteShort) + '">'
      : '<span class="brand-mark">' + esc((cfg.brandText || cfg.siteShort.slice(0, 1))) + "</span>") +
  ...
  const fLinks = (cfg.pages || []).filter(p => p.key !== "contact")
    .map(p => "<li><a href=\"" + o.pre + p.key + ".html\">" + esc(p.label) + "</a></li>").join("");
  const fNews = (cfg.categories || []).map(c => "<li><a href=\"" + o.pre + "news/cat-" + c.key + ".html\">" + esc(c.label) + "</a></li>").join("");

  return fill(tpl, {
    TITLE: o.title,
    META_DESC: o.desc,
    FAVICON: o.pre + "favicon.ico",
    CSS: o.pre + "css/style.css",
    PAGE: o.page,
    PRE: o.pre,
    BRAND: brand,
    SITE_SHORT: esc(cfg.siteShort),
    SITE_FULL: esc(cfg.siteFull),
    NAV: navHtml(cfg, o.pages, o.pre, activeKey(o.page)),
    CONTENT: o.content,
    DISCLAIMER: esc(cfg.disclaimer || "") + '（<a href="' + o.pre + 'terms.html">《使用条款》</a>）',
    CONTACT: contact,
    LINKS: links,
    F_LINKS: fLinks,
    F_NEWS: fNews,
...
function newsListItem(post, pre) {
  return (
    ...
    (post.image
      ? '<a class="news-thumb" href="' + pre + post.url + '"><img src="' + pre + post.image + '" alt="' + esc(post.title) + '" loading="lazy"></a>'
      : "") +
    '<div class="news-main">' +
...
function homePage(cfg, posts, pages) {
  ...
  const hero =
    '<section class="hero"' + (img ? ' style="background-image:url(\'' + esc(img) + '\')"' : "") + ">" +
    '<div class="hero-inner"><h1 class="hero-title">' + esc(cfg.siteShort) + "</h1></div></section>";

  const catNews = posts.filter(p => p.catKey === "news");
  const catNewsKey = (cfg.categories[0] || { key: "news" }).key;
  const featured = catNews.filter(p => p.image).slice(0, 3);
  ...
  const featHtml = featured.length
    ? '<div class="feat-grid">' + featured.map(p =>
        '<a class="feat" href="' + p.url + '"><span class="feat-img"><img src="' + p.image + '" alt="' + esc(p.title) + '" loading="lazy"></span>' +
  ...
  const newsCol =
    ...
    '<a class="more" href="news/cat-' + catNewsKey + '.html">更多 →</a></div>' +
  ...
  const noticesCol =
    ...
    '<a class="more" href="news/cat-notices.html">更多 →</a></div>' + noticeHtml + "</div>";
...
function singlePage(cfg, page, pages) {
  ...
  const content =
    ...
    '<div class="article-content">' + rootify(page.contentHtml, 0) + "</div></div></div></section>";
  return shellHtml(cfg, {
...
function articlePage(cfg, post, pages) {
  ...
  const content =
    ...
    (post.image
      ? '<figure class="article-cover"><img src="' + pre + post.image + '" alt="' + esc(post.title) + '"></figure>'
      : "") +
...
function searchPage(cfg, posts, pages) {
  ...
  const content =
    ...
    '<p><a class="btn btn-ghost" href="' + pre + 'index.html">← 返回首页</a></p></div>' +
    "</div></section>";
  ...
  const script = [
    "<script>",
    "window.__POSTS__ = " + JSON.stringify(data) + ";",
    "(function () {",
...
function sitemapXml(posts, pages, cfg) {
  const urls = ["index.html", "search.html"];
...
function main() {
  ...
  for (const p of pages) write(path.join(OUT_DIR, p.url), singlePage(cfg, p, pages));

  write(path.join(OUT_DIR, "search.html"), searchPage(cfg, posts, pages));
